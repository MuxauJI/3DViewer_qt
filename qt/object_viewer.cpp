...
void Object_viewer::init_facets() {
  object_viewer_free();  //  освобождаем памяь и обнуляем все значения в
                         //  структуре
...
void Object_viewer::update_frame() { update(); }
...
void Object_viewer::rotate_viewer(int angle, char direction) {
  int current_rotation = (direction == 'x')   ? rotation[0]
...
void Object_viewer::move_viewer(int distance, char direction) {
  int current_position = (direction == 'x')   ? position[0]
                         : (direction == 'y') ? position[1]
                         : (direction == 'z') ? position[2]
                                              : '0';
